...
class KanjiEditionDialog extends JDialog implements EditionDialog
{
	...
	private void initialize() {
        ...
        if (kanji != null)
        {
        	...
        	this.jTextFieldLecturesChinoises.setText(kanji.getLecturesChinoises());
        	this.jTextFieldLecturesJaponaises.setText(kanji.getLecturesJaponaises());
        	this.jTextFieldSignifications.setText(kanji.getSignifications());
        	this.jTextFieldThemes.setText(kanji.getThemes());
        	this.jTextFieldTrace.setText(kanji.getOrdreTraits());
        }
	}
	...
	public synchronized boolean afficher()
	{
		...
		pack();
		/*
		Point loc = app.getJFrame().getLocation();
		loc.translate(20, 20);
		editionDialog.setLocation(loc);
		*/
		setVisible(true);
		return kanjiEdite;
	}
